...
| 模块代码      | 模块名称     | 优先级 | 依赖         | 预估工时 |
| ------------- | ------------ | :----: | ------------ | -------- |
| **COLLECTOR** | Go 采集器    | 🔴 高  | 新建 Go 项目 | 2-3 周   |
| **SSH**       | SSH 设备管理 | 🔴 高  | Go 采集器    | 1 周     |
| **DISCOVERY** | 自动发现     | 🟡 中  | Go 采集器    | 1 周     |
| **MOBILE**    | 移动端 H5    | 🟡 中  | -            | 1 周     |
| **HA**        | 多活灾备     | 🟢 低  | 基础设施     | 2 周     |
...
- [ ] License 后端强校验（设备点数、用户并发）
- [ ] License 前端动态渲染（根据模块权限显隐菜单）
- [ ] 运维脚本（install.sh, backup.sh, upgrade.sh）
...
| -------------------- | :----: | -------------------- |
| 部分后端使用模拟数据 | 🟡 中  | 功能演示受限         |
| 缺少 Go 采集层       | 🔴 高  | 无法真实采集设备数据 |
| 缺少 Redis 缓存      | 🟢 低  | 高并发场景性能受限   |
| 缺少 TimescaleDB     | 🟡 中  | 时序数据存储不够专业 |
